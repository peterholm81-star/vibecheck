...
import { MapPin, TrendingUp, Users, Heart, Flame, Zap, Calendar, Sparkles, PartyPopper } from 'lucide-react';
...
export function VenueList({ venues, checkIns, timeWindowMinutes, heatmapMode, activeAgeBands, activeIntents, onVenueClick }: VenueListProps) {
  ...
  const venuesWithStats = useMemo(() => {
    return calculateAllVenueStats(venues, checkIns);
  }, [venues, checkIns]);
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-slate-800 rounded-xl border border-slate-700">
        <div className="text-center p-8">
          ...
          <p className="text-slate-300 text-lg font-medium">Ingen steder tilgjengelig</p>
          <p className="text-slate-500 text-sm mt-2">Kom tilbake senere</p>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-slate-800 rounded-lg border border-slate-700 p-4 space-y-3">
        ...
        <div className="flex items-center justify-between text-sm">
          <span className="text-slate-400">
            <strong className="text-white">{venues.length}</strong> venues
          </span>
          <span className="text-slate-400">
